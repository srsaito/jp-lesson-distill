...
- **Offline fixtures** (gitignored, local only): `work/ref-windowed/w{1,2,3}/<date>/transcript.json` are windowed transcripts that are **known-good on the words and known-BAD on the speakers** — ground truth caught their labels inverted in blocks, 0 for 16 on contested lines (jld-dli). Use them for coverage/density/merge tests, never as a diarization reference. `offset.txt` = seconds to add to reach absolute time; `work/20260729/pass_a_partial.attempt1.json` is a real repetition-loop sample. Unit-test parsing, merging, de-dup, loop detection and sanity gates against these — **never against the live API**.
