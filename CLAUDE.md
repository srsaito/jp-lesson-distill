...
- **Offline fixtures** (gitignored, local only — archived copy in OneDrive, see `docs/archive.md`): `work/ref-windowed/w{1,2,3}/<date>/transcript.json` are windowed transcripts that are **known-good on the words and known-BAD on the speakers** — ground truth scored them 0 for 16 on contested lines (jld-dli). Use them for coverage/density/merge tests, never as a diarization reference. Worth knowing *why* they are still useful: they were produced by the same prompt and code as the good run, three days apart, so they are evidence that diarization varies run-to-run rather than evidence of a broken code path. `offset.txt` = seconds to add to reach absolute time; `work/20260729/pass_a_partial.attempt1.json` is a real repetition-loop sample. Unit-test parsing, merging, de-dup, loop detection and sanity gates against these — **never against the live API**.
...
- **Model is PINNED (ADR-0006):** `gemini-3.1-pro-preview`, not a `-latest` alias. An alias moves — this one served 2.5 Pro when the pipeline was built and 3.1 Pro now — and `moments.json` then records the name of nothing, which is why two runs of the same lesson could not be compared (`jld-dli`). `DEFAULT_MODEL` in `gemini.py` is the only model id in the source; the CLI and `Config` import it. Every call prints the version the API actually served **when it differs** from the id asked for, so an alias still tells you what it resolved to.
- **Thinking config (ADR-0007):** Pass A sends `thinking_level=medium`; detect and Pass B send no level. **Do not "optimise" Pass A down to `low`** — it looks right (transcription is mechanical) and it is measured wrong: three `low` runs over one real 20-minute window got **6 of 15** address-form cue lines wrong, clean lines that can only be the student labelled `teacher`, where the run that scored 89% against blind ground truth got 5/5. Deciding *who spoke* is the inference half of Pass A, and it is what `low` spends. Pinned in `tests/test_thinking_level_measurement.py`. Never `thinking_budget` (silently ignored on 3.x, and a 400 alongside a level); `MINIMAL` is a 400 on this model.
- Thinking and answer share the 65,536-token output window. Every call logs the split and warns above 25% of it — the share at which thinking is measurably competing with the transcript (`medium` sits at 14%, ~9,300 tokens). Treat `None` as **unknown, not zero**: on a 20-minute window at `low` the API reports no thinking count at all, so a missing field read as 0 would "confirm" a setting that may have done nothing.
...
- Cost: the Gemini Project has a **$100/mo cap**. A 20-minute window streams in ~100 s at `medium` (first byte 59 s; `low` answers in 4 s, the model's own default in 71 s), so a clean lesson is ~5 min of wall clock against the ~30 the single hour-long call took. The intermittent stall (`jld-hc9.3`) adds one idle timeout per retry when it hits, at any thinking level. Don't burn lessons on experiments — use the synthetic lesson or a single window (`work/20260817/audio.m4a` is cached; `--window-minutes 20` cuts window 1 for you).
